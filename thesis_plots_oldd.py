...
def fig_spectrum(freq, amp, *, f_pass, fmax=None, offset=None,
                 lines=None, line_amps=None, label=r"$F_{\mathrm{tr}}$",
                 stem="spectrum", header=None, width=TEXT_WIDTH,
                 log=True, n_comb=8):
    """Amplitude spectrum with the Floquet comb marked.

    Vertical guides are drawn at ``n * f_pass`` (the comb from the periodic
    supports) and, if ``offset`` is given, at ``+/-offset + n * f_pass`` —
    the lines a Floquet solution must produce. A response whose peaks land
    on that comb is the spectral signature of the instability.

    Parameters
    ----------
    freq, amp : array_like
        Frequency [Hz] and normalised amplitude.
    f_pass : float
        Support-passing frequency [Hz]; sets the comb spacing.
    fmax : float or None
        Upper limit of the plotted band.
    offset : float or None
        Comb offset f0 (0 for 1T, f_pass/2 for 2T, else incommensurate).
    lines, line_amps : array_like or None
        Detected peaks, marked with dots.
    log : bool
        Log amplitude axis — essential for seeing weak comb members.
    """
    ...
    # Support-passing comb: integer multiples of f_pass
    for n in range(0, n_comb + 1):
        f = n * f_pass
        if f > fmax:
            break
        ax.axvline(f, color=C["neutral"], lw=0.5, ls="-", alpha=0.55, zorder=0)

    # Floquet comb: +/-offset + n * f_pass
    if offset is not None and offset > 1e-9:
        drawn = False
        for n in range(0, n_comb + 1):
            for f in (offset + n * f_pass, -offset + n * f_pass):
                if 0 < f <= fmax:
                    ax.axvline(f, color=C["tongue"], lw=0.7, ls="--",
                               alpha=0.8, zorder=1,
                               label=None if drawn else
                               r"$\pm f_0 + n f_{\mathrm{pass}}$")
                    drawn = True

    floor = amp[band][amp[band] > 0].min() if np.any(amp[band] > 0) else 1e-12
    if log:
        ax.semilogy(freq[band], np.maximum(amp[band], floor),
                    color=C["force"], lw=0.8,
    ...
    if lines is not None and len(lines) and line_amps is not None:
        ...
        ax.plot(np.asarray(lines)[sel], np.asarray(line_amps)[sel], "o",
                color=C["mass1"], ms=3.5, mfc="white", mew=0.9,
                lw=0, zorder=4, label="Detected lines")

    # Secondary axis in units of f_pass — how the comb is usually read
...
def fig_spectrum_multi(series, *, f_pass, fmax=None, offset=None,
                       stem="spectrum_multi", header=None,
                       width=TEXT_WIDTH, n_comb=8):
    """Overlay several spectra (e.g. F_tr, z1, z2) on one axis.

    Useful for combination resonance: the two participating modes show
    different relative strengths in z1 and z2, which is direct evidence
    that both DOFs take part.

    Parameters
    ----------
    series : list of (freq, amp, label)
        One tuple per signal.
    """
    fig, ax = _new(width, ratio=0.5)
